    ...
    return(
        ...
        <Card className="p-3 mb-3 post" onClick={!isSinglePost ? handleClick : undefined}>
            ...
            <div className="post-container" >
                <p>{post.reposted_by && <small>Reposted by: <a href={`/${post.reposted_by}`}>@{post.reposted_by}</a></small>}</p>
                <p>{post.replying_to && <a href={`/post/${post.reply_to}`}><small>In response to @'s post{post.replying_to}</small></a> }</p>
                ...
                <div className="post-buttons d-flex flex-row" onClick={(ev) => ev.stopPropagation()}>
                    <Button variant={(!liked ? 'primary' : 'danger')} onClick={likePost}>{!liked ? 'I like' : 'I do not like it anymore'}</Button>
                    <Button className="ml-3" onClick={repostPost} variant={(!reposted ? 'primary' : 'danger')}>{!reposted ? 'Repost' : 'undo repost'}</Button>
                    <DropdownButton className="ml-3"id="dropdown-basic-button" title="Actions">
                        <Dropdown.Item href="#">Share</Dropdown.Item>
                        {(post.username === user.username) ? <Dropdown.Item onClick={() => setShowingDeletePostModal(true)}>Eliminate</Dropdown.Item> : null}
                        {(post.username !== user.username) ?<Dropdown.Item href="#">Report</Dropdown.Item> : null}
                    </DropdownButton>
                </div>
                <div className="d-flex flex-row">
                    <small className="p-3 underlined-clickable-text" onClick={() => setShowingLikesModal(true)}>I like: {post.likes}</small>
                    <small className="p-3 underlined-clickable-text" onClick={() => setShowingRepostsModal(true)}>Reposts: {post.reposts}</small>
                    <small className="p-3">Answers: {post.replies}</small>
